  ...
  (function () {
    ...
    this.$cellWidthsForRow = function (row) {
      ...
      for (var i = 0, len = tabs.length - 1; i < len; i++) {
        ...
        var rightmostSelection = this.$rightmostSelectionInCell(
          selectionColumns,
          rightEdge
        );
        var cell = line.substring(leftEdge, rightEdge);
        widths[i] = Math.max(
          cell.replace(/\s+$/g, "").length,
          rightmostSelection - leftEdge
        );
      }
      return widths;
    };
    ...
    this.$rightmostSelectionInCell = function (
      selectionColumns,
      cellRightEdge
    ) {
      var rightmost = 0;
      if (selectionColumns.length) {
        ...
        for (var s = 0, length = selectionColumns.length; s < length; s++) {
          if (selectionColumns[s] <= cellRightEdge) lengths.push(s);
          else lengths.push(0);
        }
    ...
    this.$adjustRow = function (row, widths) {
      ...
      for (var i = 0, l = expandedSet.length; i < l; i++) {
        ...
        if (difference < 0 && ispaces >= -difference) {
          this.$editor.session
            .getDocument()
            .removeInLine(row, it + difference, it);
          bias += difference;
        }
      }
    };
