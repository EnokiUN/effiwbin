...
define("ace/mode/sqlserver_highlight_rules", [
  "require",
  "exports",
  "module",
  "ace/lib/oop",
  ...
  var SqlServerHighlightRules = function () {
    ...
    keywords = keywords.filter(function (value, index, self) {
      return (
        logicalOperators.split("|").indexOf(value) === -1 &&
        builtinFunctions.split("|").indexOf(value) === -1 &&
        dataTypes.split("|").indexOf(value) === -1
      );
    });
    ...
    var isolationLevels =
      "READ UNCOMMITTED|READ COMMITTED|REPEATABLE READ|SNAPSHOP|SERIALIZABLE".split(
        "|"
      );
    for (var i = 0; i < isolationLevels.length; i++) {
      setStatements.push(
        "SET TRANSACTION ISOLATION LEVEL " + isolationLevels[i]
      );
    }
    this.$rules = {
      start: [
...
define("ace/mode/folding/sqlserver", [
  "require",
  "exports",
  "module",
  "ace/lib/oop",
...
define("ace/mode/sqlserver", [
  ...
  var SqlServerHighlightRules =
    require("./sqlserver_highlight_rules").SqlHighlightRules;
