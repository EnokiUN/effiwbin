define("ace/mode/vbscript_highlight_rules", [
  "require",
  "exports",
  "module",
  "ace/lib/oop",
    ...
    var keywordMapper = this.createKeywordMapper(
      {
        ...
        "support.constant.asp":
          "TotalBytes|Buffer|CacheControl|Charset|ContentType|Expires|ExpiresAbsolute" +
        ...
        "support.function.vb.asp":
          "Array|Add|Asc|Atn|CBool|CByte|CCur|CDate|CDbl|Chr|CInt|CLng" +
        ...
        "support.type.vb.asp":
          "vbTrue|vbFalse|vbCr|vbCrLf|vbFormFeed|vbLf|vbNewLine|vbNullChar|vbNullString" +
...
define("ace/mode/folding/vbscript", [
  "require",
  "exports",
  "module",
  "ace/lib/oop",
  ...
  (function () {
    ...
    this.getFoldWidgetRange = function (session, foldStyle, row) {
      ...
      if (isStart || isEnd) {
        var match = isEnd
          ? this.foldingStopMarker.exec(line)
          : this.foldingStartMarker.exec(line);
        var keyword = match && match[1].toLowerCase();
        if (keyword) {
          var type = session.getTokenAt(row, match.index + 2).type;
          if (
            type === "keyword.control.asp" ||
            type === "storage.type.function.asp"
          )
    ...
    this.vbsBlock = function (session, row, column, tokenRange) {
      ...
      switch (val) {
        ...
        case "with":
          var line = session.getLine(row);
          var singleLineCondition = /^\s*If\s+.*\s+Then(?!')\s+(?!')\S/i.test(
            line
          );
            ...
            if (token) {
              ...
              if (val == "end") {
                ...
                firstRange = new Range(
                  firstRange.start.row,
                  firstRange.start.column,
                  tokenRange.start.row,
                  tokenRange.end.column
                );
              }
            }
            dir = -1;
            ...
            switch (val) {
              case "end":
                ...
                if (token) {
                  ...
                  if (val in endOpenings) {
                    if (
                      startTokenValue == "else" ||
                      startTokenValue == "elseif"
                    ) {
                      if (val !== "if") {
                        ranges.shift();
                      }
...
*/
  ...
  var VBScriptHighlightRules =
    require("./vbscript_highlight_rules").VBScriptHighlightRules;
  ...
  (function () {
        ...
        if (
          ...
          if (val in indentKeywords) {
            switch (val) {
              ...
              case "if":
                var checkToken = new RegExp("^\\s* end\\s+" + val, "i");
                var singleLineCondition =
                  /^\s*If\s+.*\s+Then(?!')\s+(?!')\S/i.test(line);
    ...
    this.checkOutdent = function (state, line, input) {
      ...
      return (
        (tokens[0].type == "keyword.control.asp" ||
          tokens[0].type == "storage.type.function.asp") &&
        outdentKeywords.indexOf(val) != -1
      );
    };
